angular.module('App').factory("request", function ($http, $cookies) {
	...
	obj.getAllProductByPage = function (page, limit, q, category_id, sort) {
		return $http.get(
			api_base + 'getAllProductByPage?page='+page+'&limit='+limit+
			'&col='+sort.column+'&ord='+sort.order+'&q='+q+'&category_id='+category_id
		);
	};
	...
	obj.getAllProductOrderByPage = function (page, limit, q) {
		return $http.get(api_base + 'getAllProductOrderByPage?page=' +page+'&limit='+limit+'&q='+q, config);
	};
